...
if (empty($error)){
  ...
  if (($extension="jpg") && ($extension="jpeg") && ($extension="gif")) { 
    ...
    $query_cek  ="SELECT * FROM presence WHERE employees_id='$row_user[id]' AND presence_date ='$date'";
     $result_cek = $connection->query($query_cek);
     ...
     if($result_cek->num_rows > 0){
      ...
      if($row_u['radius'] > $radius){
        ...
        $query  ="SELECT employees_id,time_in,time_out,status FROM presence WHERE employees_id='$row_u[id]' AND status= 1";
          ...
          if($row['status'] == 1) {
              if($row['time_out']=='00:00:00'){
                      ...
                      /* -------- Upload Foto Pulang -------*/
                        ...
                        // ambil tanggal data absen masuk user 
                        $query_date  ="SELECT employees_id,time_in,time_out,presence_date FROM presence WHERE employees_id='$row_u[id]' AND status=1";
                        
                        $result_date = $connection->query($query_date);
                        $row_date = $result_date->fetch_assoc();
                        
                          $update ="UPDATE presence SET time_out='$time',picture_out='$filename',latitude_longtitude_out='$latitude',produksi='$produksi', status=0, date_out='$date' WHERE employees_id='$row_u[id]' AND presence_date='$row_date[presence_date]'";
                          if($connection->query($update) === false) { 
                              die($connection->error.__LINE__); 
